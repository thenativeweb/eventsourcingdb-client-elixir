defmodule EventsourcingdbTest do
  ...
  test "ping", %{esdb: esdb} do
    assert Eventscourcingdb.ping(TestContainer.get_client(esdb)) == :ok
  end
  ...
  test "ping unavailable server" do
    ...
    result = Eventscourcingdb.ping(client)

    assert match?({:error, :transmission_error, _}, result)
  end

  test "verify api token", %{esdb: esdb} do
    assert Eventscourcingdb.verify_api_token(TestContainer.get_client(esdb)) == :ok
  end
  ...
  test "verify invalid api token", %{esdb: esdb} do
    ...
    result = Eventscourcingdb.verify_api_token(invalid_client)
